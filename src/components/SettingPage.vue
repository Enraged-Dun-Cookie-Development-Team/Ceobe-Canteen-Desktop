...
import operate, { ClearStatus } from "@/api/operations/operate";
  ...
  <div class="setting">
    <v-card>
      ...
      <SettingItem title="缓存">
        <template #sub-title> 当前缓存: {{ setting.cacheSize }} </template>
        <template #action="{ color }">
          <v-btn
            v-if="cacheClearManager.status.status === 'Done'"
            color="success"
            @click="setting.getCacheSize"
          >
            清理完成
            <v-tooltip activator="parent" location="top"
              >已清理 {{ cacheClearManager.status.data }} 个文件</v-tooltip
            >
          </v-btn>
          <v-btn
            v-else-if="cacheClearManager.status.status === 'Error'"
            color="error"
          >
            错误发生
            <v-tooltip activator="parent" location="top"
              >清理缓存时出现异常，无法正常清理</v-tooltip
            >
          </v-btn>
          <v-btn
            v-else
            v-bind="props"
            :color="color"
            :loading="cacheClearManager.status.status === 'Clearing'"
            :aria-disabled="cacheClearManager.status.status === 'Clearing'"
