...
<template>
  <div class="donate">
    <v-card>
      ...
      <v-card-text>
        <div v-if="force" class="text-subtitle-1 pl-6">更新方式：强制</div>
      ...
      <v-card-actions class="justify-space-around flex-wrap">
        <v-btn color="#e6a23c" @click="version.openUrl(versionInfo.exe)"
          >Windows安装包
        </v-btn>
        <v-btn color="#e6a23c" @click="version.openUrl(versionInfo.spare_exe)"
          >Windows备用安装包
        </v-btn>
        <v-btn color="#e6a23c" @click="version.openUrl(versionInfo.dmg)"
          >Mac安装包
        </v-btn>
        <v-btn color="#e6a23c" @click="version.openUrl(versionInfo.spare_dmg)"
          >Mac备用安装包
        </v-btn>
        <v-btn color="#e6a23c" @click="version.openUrl('')"
          >Ubuntu安装包
        </v-btn>
        <v-btn color="#e6a23c" @click="version.openUrl('')"
          >Ubuntu备用安装包
        </v-btn>
        <v-btn color="#e6a23c" @click="version.openUrl(versionInfo.baidu)"
          >百度云链接{{ versionInfo.baidu_text }}
        </v-btn>
        <v-btn v-if="force" color="red" @click="version.exist"
          >退出小刻食堂
        </v-btn>
      </v-card-actions>
    </v-card>
