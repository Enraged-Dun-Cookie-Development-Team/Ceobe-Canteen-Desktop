...
import {
    DatasourceItem, getResourceList,
...
export class BackgroundRunner {
    ...
    public static async init(): Promise<BackgroundRunner> {
        ...
        if (combine_id == null) {
            ...
            let uuids: string[] = resource_data.data.data.map((item: DatasourceItem) => item.unique_id,);
    ...
    public async run() {
        this.unlistenUpdate = await datasourceConfigOperate.datasourceCombUpdated(async () => {
    ...
    private async fetchCookie() {
        try {
            ...
            if (cookie_id != null && (cookie_id !== this.old_cookie_id || update_cookie_id !== this.old_update_cookie_id)) {
                console.log(this.datasource_comb_id, cookie_id, update_cookie_id);
                await logger.info("backgroundUtil", {
                    "combineId": this.datasource_comb_id, "cookieId": cookie_id, "updateCookieId": update_cookie_id
                });
                let cookies_data = await getCookieList(this.datasource_comb_id, cookie_id, update_cookie_id,);
                let cookies_info: TimelineData = cookies_data.data.data;
                ...
                cookies_info.cookies.forEach((cookie) => {
                    if (!this.old_cookie_id_map.get(`${cookie.source.type}:${cookie.source.data}:${cookie.item.id}`,)) {
                        this.old_cookie_id_map.set(`${cookie.source.type}:${cookie.source.data}:${cookie.item.id}`, true,);
